...
{
  downloadStyledSampleXlsx('biblio_item_sample_import', [
...
$expected_item_columns = [
    'item_code','call_number','coll_type_name','inventory_code','received_date','supplier_name','order_no','location_name',
    'order_date','item_status_name','site','source','invoice','price','price_currency','invoice_date','input_date','last_update','title'
];
$expected_item_column_count = count($expected_item_columns);
...
if (isset($_POST['doImport'])) {
    ...
    if (!isset($_POST['process'])) {
        ...
        // received_date(4), order_date(8), invoice_date(15), input_date(16), last_update(17)
        $rows = fixExcelSerialDates($rows, [4, 8, 15, 16, 17], isset($_SESSION['csv']['header']));
        ...
        try {
            ...
            while (!feof($file)) {
                ...
                if ($row_count < $record_offset) {
                    ...
                    $field = fgetcsv($file, $max_chars, $field_sep, $field_enc);
                    if ($field) {
                        if (isset($_SESSION['csv']['header']) && $row_count === $record_offset) {
                            $row_count++;
                            continue;
                        }

                        if (count($field) < $expected_item_column_count) {
                            $row_number = $row_count + 1;
                            $columns = implode(', ', $expected_item_columns);
                            throw new Exception("Format impor eksemplar tidak sesuai pada baris {$row_number}. Ditemukan " . count($field) . " kolom, seharusnya {$expected_item_column_count} kolom. Urutan kolom yang benar: {$columns}");
                        }
                        ...
                        if ($title === '') {
                            $row_number = $row_count + 1;
                            throw new Exception("Import eksemplar gagal pada baris {$row_number}. Kolom title wajib diisi karena dipakai untuk mencocokkan data bibliografi.");
                        }
                        $coll_type_name = trim((string) ($field[2] ?? ''));
                        $supplier_name = trim((string) ($field[5] ?? ''));
                        $location_name = trim((string) ($field[7] ?? ''));
                        $item_status_name = trim((string) ($field[9] ?? ''));

                        $field[2] = $coll_type_name === '' ? null : utility::getID($dbs, 'mst_coll_type', 'coll_type_id', 'coll_type_name', $coll_type_name, $ct_id_cache);
                        $field[5] = $supplier_name === '' ? null : utility::getID($dbs, 'mst_supplier', 'supplier_id', 'supplier_name', $supplier_name, $spl_id_cache);
                        $field[7] = $location_name === '' ? null : utility::getID($dbs, 'mst_location', 'location_id', 'location_name', $location_name, $loc_id_cache);
                        $field[9] = $item_status_name === '' ? null : utility::getID($dbs, 'mst_item_status', 'item_status_id', 'item_status_name', $item_status_name, $stat_id_cache);
                        ...
                        if($b_q->num_rows < 1) {
                            $row_number = $row_count + 1;
                            VarDumper::dump("Baris {$row_number} dilewati: title '{$title}' tidak ditemukan di data bibliografi.");
                            $row_count++;
        ...
        $files_disk->delete('temp' . DS . $fileName . '.xlsx');
...
<div class="menuBox">
    <div class="menuBoxInner importIcon">
        ...
        <div class="infoBox">
            <?php echo __('Import for item data from XLSX file'); ?>
