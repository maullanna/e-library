  ...
  if (!isset($_POST['process'])) {
    ...
    try {
      ...
      $reader->each(formatter: function (&$field, $row, $index) use ($dbs) {
        ...
        if ($state->execute($fields) && $state->rowCount() > 0) {
          VarDumper::dump(
          ...
          VarDumper::dump(
            str_replace('{title}', $fields[0] ?? '?', __('Skipped importing bibliographic data : {title} (duplicate or invalid data)'))
          );
        }
...
<div class="menuBox">
  <div class="menuBoxInner importIcon">
    ...
    <div class="infoBox">
      <?php echo __('Import for bibliographics data from XLSX file. For guide on fields order and format please refer to documentation or visit <a href="http://slims.web.id" target="_blank">Official Website</a>'); ?>
