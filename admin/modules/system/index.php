...
if (isset($_POST['updateData'])) {
    ...
    $imagesConfig = [
      'image' => [
        'filename' => 'logo',
        'extension' => ['.ico','.png','.jpg','.jpeg','.gif'],
        'max' => 1024*1024, // 1 MB untuk logo
        'configname' => 'logo_image'
      ],
      'icon' => [
        'filename' => 'webicon',
        'extension' => ['.ico','.png','.jpg','.jpeg','.gif'],
        'max' => 1024*1024, // 1 MB untuk favicon
        'configname' => 'webicon'
      ]
    ];
    ...
    foreach ($_FILES as $name => $detail) {
      ...
      if (empty($_FILES[$name]['name'])) {
        error_log("File $name is empty or not uploaded");
        continue;
      }
      
      // Debug: Log file details
      error_log("Processing file $name: " . print_r($detail, true));
      ...
      if ($imagesDisk->getUploadStatus()) {
        ...
        utility::jsToastr(__('System Configuration'), __('Image uploaded successfully') . ': ' . $config['filename'], 'success');
      } else {
          $errorMessage = $imagesDisk->getError();
          $fileInfo = $_FILES[$name];
          $fileSize = round($fileInfo['size'] / 1024, 2);
          $maxSize = round($config['max'] / 1024, 2);
          $allowedExts = implode(', ', $config['extension']);
          
          // Debug: Get file extension
          $fileExtension = strtolower('.' . pathinfo($fileInfo['name'], PATHINFO_EXTENSION));
          $isExtensionAllowed = in_array($fileExtension, $config['extension']);
          
          $detailedError = __('Image Upload Failed') . ': ' . $config['filename'] . '<br/>';
          $detailedError .= __('File name') . ': ' . $fileInfo['name'] . '<br/>';
          $detailedError .= __('File extension') . ': ' . $fileExtension . ' (' . ($isExtensionAllowed ? 'ALLOWED' : 'NOT ALLOWED') . ')<br/>';
          $detailedError .= __('File size') . ': ' . $fileSize . ' KB (' . __('Maximum allowed') . ': ' . $maxSize . ' KB)<br/>';
          $detailedError .= __('Allowed extensions') . ': ' . $allowedExts . '<br/>';
          $detailedError .= __('Error details') . ': ' . $errorMessage;
          
          utility::jsToastr(__('System Configuration'), $detailedError, 'error');
      }
    }
...
if(isset($sysconf['logo_image']) && $sysconf['logo_image'] != '' && $imagesDisk->isExists('default/'.$sysconf['logo_image'])){
    $str_input .= '<div style="padding:10px;">';

    $str_input .= '<img src="'.SWB.'lib/minigalnano/createthumb.php?filename=images/default/'.$sysconf['logo_image'].'&width=130&t='.time().'" class="img-fluid rounded" alt="Image cover">';
    ...
    // Debug: Show why logo is not displayed
    $str_input .= '<div style="padding:10px; background:#f8f9fa; border:1px solid #dee2e6; border-radius:4px; font-size:12px;">';
    $str_input .= '<strong>Debug Info:</strong><br/>';
    $str_input .= 'logo_image: ' . ($sysconf['logo_image'] ?? 'NOT_SET') . '<br/>';
    $str_input .= 'file exists: ' . ($imagesDisk->isExists('default/'.$sysconf['logo_image']) ? 'YES' : 'NO') . '<br/>';
    $str_input .= 'full path: ' . SB . 'images/default/' . ($sysconf['logo_image'] ?? 'N/A') . '<br/>';
    $str_input .= '</div>';
}
$str_input .= '<div class="custom-file col-3 d-block">';
$str_input .= simbio_form_element::textField('file', 'image', '', 'class="custom-file-input" id="customFileImage"');
$str_input .= '<label class="custom-file-label" for="customFileImage">'.__('Choose file').'</label>';
$str_input .= '</div>';
$str_input .= '<div class="mt-2 ml-2">Maximum 1 MB | ' . __('Allowed formats') . ': .ico, .png, .jpg, .jpeg, .gif</div>';

// Web icon
$str_input .= '<strong class="d-block mt-2">'.__('Favicon').'</strong>';

if(isset($sysconf['webicon']) && $sysconf['webicon'] != '' && $imagesDisk->isExists('default/'.$sysconf['webicon'])){
    $str_input .= '<div style="padding:10px;">';
    $str_input .= '<img src="'.SWB.'lib/minigalnano/createthumb.php?filename=images/default/'.$sysconf['webicon'].'&width=130&t='.time().'" class="img-fluid rounded" alt="Image cover">';
...
$str_input .= simbio_form_element::textField('file', 'icon', '', 'class="custom-file-input" id="customFileIcon"');
$str_input .= '<label class="custom-file-label" for="customFileIcon">'.__('Choose file').'</label>';
$str_input .= '</div>';
$str_input .= '<div class="mt-2 ml-2">Maximum 1 MB | ' . __('Allowed formats') . ': .ico, .png, .jpg, .jpeg, .gif</div>';
$str_input .= <<<HTML
<script>
