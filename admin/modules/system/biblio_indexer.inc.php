...
class biblio_indexer
{
	...
	public function makeIndex($int_biblio_id)
	{
	    ...
	    WHERE b.biblio_id=' . intval($int_biblio_id);
		// query
		$rec_bib = $this->obj_db->query($bib_sql);
		...
		$colltype_sql = 'SELECT ct.coll_type_name AS `name`
          	...
          	WHERE i.biblio_id =' . intval($int_biblio_id);
	...
	public function updateIndex($int_biblio_id)
	{
		...
		$this->obj_db->query("DELETE FROM search_biblio WHERE biblio_id=" . intval($int_biblio_id));
	...
	protected function wordIndex($word, $count)
	{
		...
		if ($query->num_rows > 0) {
			...
			$num_hits = intval($data[1]) + intval($count);
			$doc_hits = intval($data[2]) + ($count > 1 ? 1 : intval($count));
			$id = intval($data[0]);
			$this->obj_db->query("UPDATE index_words SET num_hits=$num_hits, doc_hits=$doc_hits WHERE id=$id");
			return $id;
		}
