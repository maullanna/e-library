...
// Development mode check for production
if (!defined('DEVELOPMENT_MODE')) {
    ...
    if (file_exists(__DIR__ . '/.env')) {
        ...
        define('DEVELOPMENT_MODE', (isset($env['DEVELOPMENT_MODE']) && $env['DEVELOPMENT_MODE'] === 'true'));
    } else {
        define('DEVELOPMENT_MODE', false); // Default to production
    }
}
...
// Environment config - using .env file only
...
if (file_exists(__DIR__ . '/.env')) {
    ...
    define('ENVIRONMENT', isset($env['ENVIRONMENT']) ? $env['ENVIRONMENT'] : 'production');
} else {
    define('ENVIRONMENT', 'production');
}

// Environment is already set to production from .env file
...
// Error handler will be registered after environment is defined
...
$localisation->load(function($memory){
  ...
  if (isset($_GET['select_lang'])) {
    ...
    if (isset($_COOKIE['select_lang'])) $memory->forgetTempLanguage($_GET['select_lang']);
    
    // make it one
    $memory->rememberTempLanguage($_GET['select_lang']);
...
/* default application language - will be overridden by database settings */
...
/* CONTENT */
$sysconf['library_name'] = 'E-Library AKTI';
$sysconf['library_subname'] = 'Perpustakaan Digital Akademi Komunitas Teknologi Indonesia';
...
$sysconf['OAI']['Identify']['baseURL'] = 'https://e-library.akti.ac.id/oai.php';
$sysconf['OAI']['Identify']['repositoryName'] = 'SLiMS Senayan Library Management System Repository';
$sysconf['OAI']['Identify']['adminEmail'] = 'admin@akti.ac.id';
...
// update locale after all settings are loaded
$localisation->updateLocaleFromConfig();
